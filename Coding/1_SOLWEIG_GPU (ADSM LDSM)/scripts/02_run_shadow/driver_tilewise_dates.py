# -*- coding: utf-8 -*-
"""Tile-wise driver (date-parameterised version: environment variables SW_DATE / SW_OUT / SW_MET; otherwise identical to driver_tilewise.py): reuses the 77 preprocessed tiles; each tile runs compute_utci in a subprocess with a timeout (hung tiles are killed/logged/skipped),
...
OUT  = os.environ.get("SW_OUT", os.path.join(BASE, "output_folder"))
DATE = os.environ.get("SW_DATE", '2026-03-01')
MET  = os.environ.get("SW_MET", "")          # forcing file for ALL tiles (per-tile metfiles are identical copies of the forcing)
...
def kwargs_for(M, key, outdir):
    return dict(
        ...
        met_file_data=np.loadtxt(MET if MET else M["met"][key], skiprows=1),
...
def main():
    ...
    print(f"total tiles: {len(keys)} | DATE {DATE} | OUT {OUT} | MET {MET or 'per-tile metfiles'}", flush=True)
