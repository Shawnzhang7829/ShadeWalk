...
| 4a | network preparation, per-edge shade and facility labels (469,434 edges; 2026-09-21 end-to-end run) | graph and shade identical on every edge to the products in use; facility labels identical on all but 5 of 426,163 main-component edges (current building-footprint mask) |
| 4b | demand-input exports and the full routing / flow chain re-run with the repository scripts (2026-09-21) | pedestrian subset, station table v4, building weights and occupancy table identical to the products in use; `flow_short` / `flow_cool` / `flow_orig` / `flow_cool_orig`, the 4 tau and 18 lambda arrays and the OD metrics identical on every edge |
