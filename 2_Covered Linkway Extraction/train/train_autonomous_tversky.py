...
def focal_tversky_loss(pred_logits, mask, alpha=0.3, beta=0.7,
                       gamma=1.3333, eps=1e-6):
    """Focal-Tversky loss.  TI = TP / (TP + a*FN + b*FP).
    beta > alpha penalises False Positives harder than False Negatives,
    directly targeting precision.  gamma>1 focuses on hard examples.
    A light BCE term keeps early gradients well-behaved.
    """
    p = torch.sigmoid(pred_logits)
    p = p.reshape(p.size(0), -1)
    g = mask.reshape(mask.size(0), -1)
    tp = (p * g).sum(1)
    fp = (p * (1 - g)).sum(1)
    fn = ((1 - p) * g).sum(1)
    ti = (tp + eps) / (tp + alpha * fn + beta * fp + eps)
    ftl = torch.pow(1.0 - ti, gamma).mean()
    bce = F.binary_cross_entropy_with_logits(pred_logits, mask)
    return ftl + 0.5 * bce
...
def main():
    ...
    p.add_argument("--loss", choices=["structure", "tversky"],
                   default="structure")
    p.add_argument("--tv_alpha", type=float, default=0.3)   # FN weight
    p.add_argument("--tv_beta",  type=float, default=0.7)   # FP weight (>alpha)
    p.add_argument("--tv_gamma", type=float, default=1.3333)
    ...
    if args.loss == "tversky":
        print(f"[init] loss = FocalTversky(a={args.tv_alpha},b={args.tv_beta},"
              f"g={args.tv_gamma}) + {args.cldice_weight}*clDice  "
    ...
    def seg_loss(logits, msks):
        if args.loss == "tversky":
            return focal_tversky_loss(logits, msks, args.tv_alpha,
                                      args.tv_beta, args.tv_gamma)
        return structure_loss(logits, msks, pos_weight=args.pos_weight)
