...
| Building footprints | `SG_Building_SVY21.gpkg` | 303,877 building polygons with a mean-height attribute (`Avg_height`), EPSG:3414; only the geometry is used (rasterised to a 1/16-resolution exclusion mask) | building layer of the lab's SOLWEIG input data, not part of the data record (it overlaps the released OpenStreetMap footprints by about 78 % of the area). To reproduce with released data, pass the shapefile in `SG_buildings_footprint_height_function.zip` instead (reprojected on load); the exclusion mask then differs slightly from the paper run | `--building_gpkg` (exclusion mask) |
| Pedestrian network | `pedestrian_network_filtered.gpkg` | 142,095 line segments, EPSG:3414 | Module 4b (`demand_inputs/export_pedestrian_subset.py`: the OSM pedestrian-passable highway subset; the cleaned OSM lines are released as `SG_osm_lines.gpkg` in the data record) | `apply_pednet_filter.py` (5 m buffer) |
