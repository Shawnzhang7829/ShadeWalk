...
| Building weights | `building_hourly_weight_gfacorr.gpkg` | 118,782 polygons, `building_archetype`, `gross_floor_area` (= `gfa_corr` of the released building dataset `SG_buildings_footprint_height_function.shp`), `weight_weekday_HH` (48 hourly columns), peaks and totals | Module 4b (`tools/building_weights_corrected_gfa.py`) | demand destinations (`weight_weekday_14`), building types in the metrics |
