...
| Station ridership | `station_hourly_ridership.gpkg` | 5,921 points (bus stops + MRT / LRT exits), `PT_CODE`, `source`, `n_exits`, hourly `in_/out_/tot_/inj_weekday_HH` and weekend columns, totals | Module 4b (`export_station_hourly.py`) | demand origins (`inj_weekday_14` = per-exit share; `tot_*` is the station total repeated on every exit row). Use an export made with the current Module 4b (interchange codes such as `NS24/NE6/CC1` split over their line codes, duplicated bus-stop codes shared); earlier exports double-count interchanges. |
