...
| Building weights | `building_hourly_weight_gfacorr.gpkg` | 118,782 polygons, `building_archetype`, `gross_floor_area` (= `gfa_corr`, storeys x footprint, of the released building dataset), `weight_weekday_HH` (48 hourly columns), peaks and totals | Module 4b (`export_building_hourly.py` + `tools/building_weights_corrected_gfa.py`; until 2026-09-21 the export on the shipped GFA, `building_hourly_weight.gpkg`) | demand destinations (`weight_weekday_14`), building types in the metrics |
