...
| 3b | `step4_4b_esn.py`, `step4_edge_facility.py`, `step4_edge_class_1m.py`, `step4_network_split_1m.py`, `edge_px_permetre.py` | no-facility shade per edge, dominant facility class per edge (arcade > linkway > tree > building), per-metre attribution of shade sources, split of the network by shade class; in the four Category-sampling scripts a class-7 sample on open water (DSMremain <= 0) counts as sun (water artefact of the Category raster) |
