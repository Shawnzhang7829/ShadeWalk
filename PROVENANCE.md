...
| `4_Pedestrian Flow Mapping/a_OD Coolest-Route Algorithm (Route Cost Calculation)/scripts/03_routing_flow/step4_4e_flow_detour.py` | 2026-09-17: optional parallel run by station subsets (`STA_SPLIT=k/K` routes every K-th station and saves a partial, `MERGE_PARTIALS=K` sums the partials and runs the unchanged post-processing); without the variables the script runs as before |
...
| `4_Pedestrian Flow Mapping/b_Pedestrian OD Flow Assignment (Gravity and Huff model)/tools/building_weights_corrected_gfa.py` (new), `4_Pedestrian Flow Mapping/a_OD Coolest-Route Algorithm (Route Cost Calculation)/scripts/03_routing_flow/step4_4c_city_routing.py`, `step4_4c_orig_flow.py`, `step4_4c_orig_coolflow.py`, `step4_4e_flow_detour.py`, `step4_4f_flow_lam.py`, `webapp/make_nav_app_maplibre.py` | 2026-09-21: corrected gross floor area. The building weights (A_j = GFA x archetype occupancy density) had used the building dataset's shipped `gross_floor_area`, which equals the footprint area for 74.3 % of the 118,782 buildings (a missing-data sentinel where the OSM storey count was absent; island total 331.5 M m2). The new tool rebuilds the weight table with `gfa_corr` of the released building dataset (resolved storeys x footprint: reported levels, else measured height / archetype floor-to-floor height, else the archetype median; 452.4 M m2; 14:00 attraction 8.37 -> 14.36 M persons, industrial / school / hospital / landed roughly doubled relative to HDB / retail / office), joined on the OSM id, everything else as the export (self-check: the shipped GFA reproduces `building_hourly_weight.gpkg`). The routing / flow scripts and the web-app builder read `building_hourly_weight_gfacorr.gpkg`; the flow products (`flow_short` / `flow_cool` / `flow_orig` columns, `flow_cool_orig`, the tau-capped and lambda flow fields, the OD-pair table) and the web app were recomputed (14:00 persons routed 620,621 -> 620,923; 42 % of the edges with flow changed by more than 25 %, citywide shaded-share statistics within 0.4 pp). The paper figures on these weights were regenerated (Fig. 3 v55, Fig. 4 v91, Fig. 5 v44); the remaining supplementary items follow. |
