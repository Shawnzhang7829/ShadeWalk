...
| `4_Pedestrian Flow Mapping/b_Pedestrian OD Flow Assignment (Gravity and Huff model)/tools/building_weights.py` (new), `4_Pedestrian Flow Mapping/a_OD Coolest-Route Algorithm (Route Cost Calculation)/scripts/03_routing_flow/step4_4c_city_routing.py`, `step4_4c_orig_flow.py`, `step4_4c_orig_coolflow.py`, `step4_4e_flow_detour.py`, `step4_4f_flow_lam.py`, `webapp/make_nav_app_maplibre.py` | 2026-09-21: building weights `building_hourly_weight_gfa.gpkg` (`tools/building_weights.py`) -- gross floor area = `gfa_corr` (storeys x footprint) of the released building dataset `5_base_data/SG_buildings_footprint_height_function.shp`, joined on the OSM id; the routing / flow scripts and the web-app builder read this table; flow products and the web app recomputed. |
