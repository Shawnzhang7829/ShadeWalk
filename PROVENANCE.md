...
|---|---|
| `1_SOLWEIG_GPU (ADSM LDSM)/sample_data/forcing/*` | removed on 2026-09-16: the five forcing files moved to the figshare data record (`1_shadow_model/forcing/`); present in the repository in releases 1.0.0 to 1.0.2 |
...
| `4_Pedestrian Flow Mapping/b_Pedestrian OD Flow Assignment (building gravity and huff model)/routing_flow/step4_4c_city_routing.py`, `step4_4c_orig_flow.py`, `step4_4c_orig_coolflow.py`, `step4_4e_flow_detour.py`, `step4_4f_flow_lam.py` | bug fix 2026-09-16: origin weight = `inj_weekday_14` (per-exit share) instead of `tot_weekday_14`, which the station export repeats on every exit row of a station (the originals injected the whole station's ridership at each exit) |
| `4_Pedestrian Flow Mapping/a_OD Coolest-Route Algorithm (route cost calculation)/webapp/make_nav_app_maplibre.py` | bug fix 2026-09-16: station dot size and hourly ratios from the per-exit shares (`tot_weekday_total / n_exits`, `inj_weekday_HH`); backup rotation now matches only the page's own timestamped copies (the old glob also matched `nav_app_en_*` / `nav_app_paper_*` and pruned the `nav_app_*` backups); the Simple basemap is now Esri World Light Gray Canvas (base + reference labels, no API key) because CARTO `light_all` tiles started to carry an 'API KEY REQUIRED' watermark; the OSM button is now the OpenFreeMap Positron vector style (no API key), whose glyphs / sprite also serve the page's own labels |
| `4_Pedestrian Flow Mapping/b_Pedestrian OD Flow Assignment (building gravity and huff model)/demand_inputs/loaders.py` (`load_bus_stations`, formerly `Patronage_Flow/Network.py`) | fix 2026-09-16: `weight_divisor` of bus stops = number of shapefile rows with the same code (18 codes occur twice), so a duplicated stop no longer injects its volume twice |
| `4_Pedestrian Flow Mapping/b_Pedestrian OD Flow Assignment (building gravity and huff model)/demand_inputs/loaders.py` (`load_ridership_split`, formerly `Patronage_Flow/Flow_Computation.py`) | fix 2026-09-17: the raw LTA CSVs are read with `dtype={'PT_CODE': str}`; parsed as integers the bus-stop codes 01xxx-09xxx lost their leading zero, never joined the stop layer and the 225 central-area stops were exported (and routed) with 0 ridership |
| `4_Pedestrian Flow Mapping/b_Pedestrian OD Flow Assignment (building gravity and huff model)/demand_inputs/station_table_real_exits.py` | new 2026-09-17: station table v4 = one record per real exit point / bus stop, values rebuilt from the raw month files (an interchange is one station, `inj_* = station total / real exit points`); replaces the per-exit x line-code rows of the export as the origin-weight table of Module 4a |
| `4_Pedestrian Flow Mapping/b_Pedestrian OD Flow Assignment (building gravity and huff model)/routing_flow/step4_4c_city_routing.py`, `step4_4c_orig_flow.py`, `step4_4c_orig_coolflow.py`, `step4_4e_flow_detour.py`, `step4_4f_flow_lam.py`, `webapp/step4_demo_scenarios.py` | 2026-09-17: station table `station_hourly_ridership_v4.gpkg` (one record per real exit point; `inj_weekday_14` = station total / real exit points) |
| `4_Pedestrian Flow Mapping/a_OD Coolest-Route Algorithm (route cost calculation)/webapp/make_nav_app_maplibre.py` | 2026-09-17: station points, labels (`CODES`, all line codes sharing an exit) and dot sizes (daily injection of the record) from station table v4 |
| `4_Pedestrian Flow Mapping/a_OD Coolest-Route Algorithm (route cost calculation)/webapp/make_nav_app_maplibre.py`, `make_en_navapp.py` | 2026-09-17: Daily time slider: the Network-view shade colouring, the coolest routing and the route cards follow the hour (edge shade layers 08-18 from `step4_4b_city_edge_shade_hourly.py`, embedded as base64 uint8; 14:00 = the canonical `shade_full`, so the default state is unchanged); facility-removal scenarios at other hours scale the 14:00 no-facility shade by the hour / 14:00 shade ratio (no hourly no-facility raster) |
| `4_Pedestrian Flow Mapping/a_OD Coolest-Route Algorithm (route cost calculation)/scripts/02_edge_shade/step4_4b_city_edge_shade.py` (data, no code change) | 2026-09-17: the production `step4_4_edges_SG.gpkg` / `step4_4_edges_flow_SG.gpkg` were regenerated with the building-footprint mask on disk (`SUB_SG_Polygon_BREMAIN_1m.tif` of the new arcade data set, which the script already referenced); the June layer had been sampled with the previous mask, so 4,403 edges (0.9 %, mostly arcade / linkway centre lines and their connections) carried a higher `shade_full` (network shade 29.09 % -> 29.02 %); `shade_full` now equals the corrected per-edge shade of the paper chain on every edge; all routing / flow products (`step4_4c*`, `step4_4e`, `step4_4f`, `od_export_app`), the hourly edge shade and the web app were recomputed |
| `4_Pedestrian Flow Mapping/b_Pedestrian OD Flow Assignment (building gravity and huff model)/routing_flow/step4_4e_flow_detour.py` | 2026-09-17: optional parallel run by station subsets (`STA_SPLIT=k/K` routes every K-th station and saves a partial, `MERGE_PARTIALS=K` sums the partials and runs the unchanged post-processing); without the variables the script runs as before |
| `4_Pedestrian Flow Mapping/a_OD Coolest-Route Algorithm (route cost calculation)/webapp/make_nav_app_maplibre.py`, `make_paper_version.py`, `make_en_navapp.py` | 2026-09-17: the Network-view shade button is labelled σₑ (edge shade fraction, tooltip); the Daily time block (label + hour slider) sits between O/D points and Basemap |
| `4_Pedestrian Flow Mapping/a_OD Coolest-Route Algorithm (route cost calculation)/webapp/make_nav_app_maplibre.py`, `make_paper_version.py`, `make_en_navapp.py` | 2026-09-17: Network view > Pedestrian flow gets a Coolest / Shortest tab strip (own line under the view buttons): Coolest = the heat-avoiding field of the current detour limit (paper page: of the current lambda), Shortest = `flow_short` (all trips on shortest routes; independent of the slider) |
| `4_Pedestrian Flow Mapping/b_Pedestrian OD Flow Assignment (building gravity and huff model)/routing_flow/step4_4c_city_routing.py`, `step4_4c_orig_flow.py`, `step4_4c_orig_coolflow.py`, `step4_4e_flow_detour.py`, `step4_4f_flow_lam.py` | 2026-09-18: main-component snapping. Station exits and demand buildings are snapped to the nearest node (<= 120 m) of the main connected component of the graph each script builds, instead of the nearest node of any component: 91 exits whose nearest node was a 2-25-node isolated fragment (no building on it) could not reach any building and were never routed (17,340 boarding persons at 14:00, 2.7 % of the origin weight). The production flow products (`flow_short` / `flow_cool` / `flow_orig` columns, `flow_cool_orig`, the tau-capped and lambda flow fields, the OD-pair table of the web tool) and the web app were recomputed (14:00 persons routed on the facility network 594,279 -> 620,621). On the footpath-only network of `step4_4c_orig_flow.py` / `step4_4c_orig_coolflow.py` the effect is larger: an exit whose nearest node of the full network was a facility node was previously dropped from the original-network flows (4,572 of 5,548 exits routed); it now snaps to the nearest footpath node of the main component (5,550 exits, 611,553 persons). The paper figures that use these flows (Fig. 3 c/d, Extended Data Table 1, Supplementary Figs. 12 and 18) were recomputed with the same rule; their shares change by <= 0.2 percentage points |
| `4_Pedestrian Flow Mapping/b_Pedestrian OD Flow Assignment (building gravity and huff model)/demand_inputs/building_weights.py` (new), `4_Pedestrian Flow Mapping/b_Pedestrian OD Flow Assignment (building gravity and huff model)/routing_flow/step4_4c_city_routing.py`, `step4_4c_orig_flow.py`, `step4_4c_orig_coolflow.py`, `step4_4e_flow_detour.py`, `step4_4f_flow_lam.py`, `4_Pedestrian Flow Mapping/a_OD Coolest-Route Algorithm (route cost calculation)/webapp/make_nav_app_maplibre.py` | 2026-09-21: building weights `building_hourly_weight_gfa.gpkg` (`tools/building_weights.py`) -- gross floor area = `gfa_corr` (storeys x footprint) of the released building dataset `5_base_data/SG_buildings_footprint_height_function.shp`, joined on the OSM id; the routing / flow scripts and the web-app builder read this table; flow products and the web app recomputed. |
...
| `4_Pedestrian Flow Mapping/b_Pedestrian OD Flow Assignment (building gravity and huff model)/demand_inputs/` (`Constants.py`, `loaders.py`, `build_occupancy_table.py`, `export_pedestrian_subset.py`, `export_station_hourly.py`, `station_table_real_exits.py`, `building_weights.py`, `lookup/`), `4_Pedestrian Flow Mapping/b_Pedestrian OD Flow Assignment (building gravity and huff model)/routing_flow/` (`step4_4c_city_routing.py`, `step4_4c_orig_flow.py`, `step4_4c_orig_coolflow.py`, `step4_4d_city_viz.py`, `step4_4e_flow_detour.py`, `step4_4f_flow_lam.py`) | 2026-09-21: Module 4 reorganised. The routing / flow scripts moved from Module 4a `scripts/03_routing_flow/` to Module 4b `routing_flow/` unchanged apart from two comment lines that name the Module 4b tools (Module 4a keeps the network preparation, the per-edge shade and the web tool). The madina-based `Patronage_Flow` package (`Flow_Computation.py`, `Main.py`, `Main_dualpass.py`, `Network.py`, `run_2025_*.py`, `export_building_hourly.py`, the notebook, `tools/patch_madina.py`, `tools/madina_smoke.py`, `docs/DEVELOPMENT_NOTES.md`) was removed: its betweenness flows were not used for any result of the paper. The three readers still needed by the demand tables (`load_pedestrian_highway`, `load_stations` with `load_bus_stations` / `load_mrt_stations`, `load_ridership_split`) moved verbatim into `demand_inputs/loaders.py`; `Constants.py` keeps the paths and parameters they use; `export_pedestrian_subset.py` (new) writes `pedestrian_network_filtered.gpkg` as `Main.py` did; `build_occupancy_table.py`, `export_station_hourly.py` and the two tools only changed their import lines and paths. Verification 2026-09-21: the exports reproduce `pedestrian_network_filtered.gpkg` (142,095 segments), `station_hourly_ridership_v4.gpkg` (5,758 records, row order aside), `building_hourly_weight_gfa.gpkg` (118,782 x 55) and `occupancy_density.parquet` value for value. |
| `4_Pedestrian Flow Mapping/a_OD Coolest-Route Algorithm (route cost calculation)/scripts/02_edge_shade/step4_edge_facility.py` | 2026-09-21: water rule of the facility labels integrated. The Category raster marks open water (DSMremain <= 0) with class 7 in places (a water artefact removed from the paper figures on 2026-09-07, Fig. 3 v47 basis); the label file in use, `edge_facility_SG.npy`, had been derived on 2026-09-13 by re-labelling the 379 affected edges (`regen_efac_v50.py`). The script now resets class-7 samples on DSMremain <= 0 to sun before the class mapping (per-point sampling of the DSM), so that one run yields the label file in use: verified 2026-09-21 on all but 5 of 426,163 edges (the 5 lie inside the current building-footprint mask of 2026-09-17, which post-dates the label file). |
| `4_Pedestrian Flow Mapping/a_OD Coolest-Route Algorithm (route cost calculation)/scripts/02_edge_shade/step4_4a_city_building_shadow.py` | 2026-09-21: raster folder `TIF` (archived) -> `TIF_shadow_newarcade` (the folder every other script reads). Run on the current folder, the building-only baseline differs from the June baseline in use on 0.014 % of the pixels (shaded area -0.11 %); the baseline in use was kept. |
| `4_Pedestrian Flow Mapping/a_OD Coolest-Route Algorithm (route cost calculation)/scripts/02_edge_shade/step4_edge_class_1m.py`, `edge_px_permetre.py`, `step4_network_split_1m.py` | 2026-09-21: the same water rule as in `step4_edge_facility.py` (Category class-7 samples on open water, DSMremain <= 0, are reset to sun before the class mapping) integrated into the three 1-m sampling scripts; their products were recomputed (`edge_class_1m_totals.csv`, `edge_px_SG.npz`, `step4_network_split_by_class.gpkg`): 13.2 km of the 1-m network length move from the covered-linkway class to sun (463.4 -> 450.2 km; 678 edges in the per-edge attribution), the other classes are unchanged. The public copies reproduce the recomputed products exactly. `step4_4b_esn.py` samples the building + tree shadow raster, not the Category raster, and is unchanged. |
| `4_Pedestrian Flow Mapping/a_OD Coolest-Route Algorithm (route cost calculation)/webapp/step4_demo_scenarios.py` | 2026-09-21: the building table it reads for the demonstration origins (archetype and footprint only) is `building_hourly_weight_gfa.gpkg`, the table of Module 4b in use; the previous path named a folder / file that no longer exists. The demonstration pairs do not depend on the weights. |
...
| `4_Pedestrian Flow Mapping/a_OD Coolest-Route Algorithm (route cost calculation)/scripts/02_edge_shade/step4_4b_city_edge_shade_hourly.py` | `D:\Claude\SVI_FFW\output\step5_nav_webapp\step4_4b_city_edge_shade_hourly.py` | new 2026-09-17 |
...
| `4_Pedestrian Flow Mapping/b_Pedestrian OD Flow Assignment (building gravity and huff model)/routing_flow/step4_4c_city_routing.py` | `D:\Claude\SVI_FFW\output\step5_nav_webapp\step4_4c_city_routing.py` |  |
| `4_Pedestrian Flow Mapping/b_Pedestrian OD Flow Assignment (building gravity and huff model)/routing_flow/step4_4c_orig_flow.py` | `D:\Claude\SVI_FFW\output\step5_nav_webapp\step4_4c_orig_flow.py` |  |
| `4_Pedestrian Flow Mapping/b_Pedestrian OD Flow Assignment (building gravity and huff model)/routing_flow/step4_4c_orig_coolflow.py` | `D:\Claude\SVI_FFW\output\step5_nav_webapp\step4_4c_orig_coolflow.py` |  |
| `4_Pedestrian Flow Mapping/b_Pedestrian OD Flow Assignment (building gravity and huff model)/routing_flow/step4_4e_flow_detour.py` | `D:\Claude\SVI_FFW\output\step5_nav_webapp\step4_4e_flow_detour.py` |  |
| `4_Pedestrian Flow Mapping/b_Pedestrian OD Flow Assignment (building gravity and huff model)/routing_flow/step4_4f_flow_lam.py` | `D:\Claude\SVI_FFW\output\step5_nav_webapp\step4_4f_flow_lam.py` |  |
| `4_Pedestrian Flow Mapping/b_Pedestrian OD Flow Assignment (building gravity and huff model)/routing_flow/step4_4d_city_viz.py` | `D:\Claude\SVI_FFW\output\step5_nav_webapp\step4_4d_city_viz.py` |  |
...
| `4_Pedestrian Flow Mapping/b_Pedestrian OD Flow Assignment (building gravity and huff model)/demand_inputs/Constants.py` | `D:\Claude\UNA\Patronage_Flow\Constants.py` | paths and parameters of the demand inputs (madina parameters removed 2026-09-21) |
| `4_Pedestrian Flow Mapping/b_Pedestrian OD Flow Assignment (building gravity and huff model)/demand_inputs/loaders.py` | `D:\Claude\UNA\Patronage_Flow\Network.py`, `D:\Claude\UNA\Patronage_Flow\Flow_Computation.py` | the three readers of the demand inputs, moved verbatim 2026-09-21 |
| `4_Pedestrian Flow Mapping/b_Pedestrian OD Flow Assignment (building gravity and huff model)/demand_inputs/export_pedestrian_subset.py` | `D:\Claude\UNA\Patronage_Flow\Main.py` (export block) | new 2026-09-21 |
| `4_Pedestrian Flow Mapping/b_Pedestrian OD Flow Assignment (building gravity and huff model)/demand_inputs/__init__.py` | `D:\Claude\UNA\Patronage_Flow\__init__.py` | demand_inputs package (formerly Patronage_Flow) |
| `4_Pedestrian Flow Mapping/b_Pedestrian OD Flow Assignment (building gravity and huff model)/demand_inputs/build_occupancy_table.py` | `D:\Claude\UNA\Patronage_Flow\build_occupancy_table.py` | demand_inputs package (formerly Patronage_Flow) |
| `4_Pedestrian Flow Mapping/b_Pedestrian OD Flow Assignment (building gravity and huff model)/demand_inputs/export_station_hourly.py` | `D:\Claude\UNA\Patronage_Flow\export_station_hourly.py` | demand_inputs package (formerly Patronage_Flow) |
| `4_Pedestrian Flow Mapping/b_Pedestrian OD Flow Assignment (building gravity and huff model)/demand_inputs/lookup/mrt_code_to_name.csv` | `D:\Claude\UNA\Patronage_Flow\lookup\mrt_code_to_name.csv` | lookup tables (MRT codes, occupancy density) |
| `4_Pedestrian Flow Mapping/b_Pedestrian OD Flow Assignment (building gravity and huff model)/demand_inputs/lookup/occupancy_density.csv` | `D:\Claude\UNA\Patronage_Flow\lookup\occupancy_density.csv` | lookup tables (MRT codes, occupancy density) |
| `4_Pedestrian Flow Mapping/b_Pedestrian OD Flow Assignment (building gravity and huff model)/demand_inputs/lookup/occupancy_density.parquet` | `D:\Claude\UNA\Patronage_Flow\lookup\occupancy_density.parquet` | lookup tables (MRT codes, occupancy density) |
...
| Repository file | Original location | Note |
|---|---|---|
| `docs/core_workflow.png` | paper_figures 0912 v4 `core_workflow_4col.pptx` slide 1 (the figure delivered as `3.pptx`), native shape group exported with PowerPoint 16.0 inside its own theme with a 6 pt transparent margin (2026-09-12) | PNG written by PowerPoint (2600 px wide, white background) |
| `docs/core_workflow.svg` | paper_figures 0912 v4 `core_workflow_4col.pptx` slide 1 (the figure delivered as `3.pptx`), native shape group exported with PowerPoint 16.0 inside its own theme with a 6 pt transparent margin (2026-09-12) | EMF written by PowerPoint, converted to plain SVG with Inkscape 1.x |
